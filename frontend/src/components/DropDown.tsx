...
dropDownData.length!==0&&valueCheck(listData.Value)&&<div className="absolute top-11 shadow-2xl flex flex-col w-[120px] px-1 py-3 rounded-lg">{dropDownData.map((el,i)=><div key={i} className="flex flex-col px-[5px] py-1 hover:bg-[#0000000d] rounded-lg"><h1 className="self-start ">{el}</h1></div>)}</div>}
</>)
}